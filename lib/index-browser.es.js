...
import nodeFetch, { Headers as Headers$1 } from 'node-fetch';
import fetchCookie from 'fetch-cookie';
import AbortController$1 from 'abort-controller';
...
function logApiCall(self, name, args) {
  ...
  if (self.constructor.listeners('debug').length) {
    ...
    args[args.length - 1] = function (err, res$$1) {
      var responseArgs = ['api', self.name, name];
      responseArgs = responseArgs.concat(
        err ? ['error', err] : ['success', res$$1]
      );
      self.constructor.emit('debug', responseArgs);
      origCallback(err, res$$1);
    };
  }
}
...
function bulkGet$$1(db, opts, callback) {
  ...
  function collapseResultsAndFinish() {
    var results = [];
    perDocResults.forEach(function (res$$1) {
      res$$1.docs.forEach(function (info) {
        results.push({
          id: res$$1.id,
          docs: [info]
        });
  ...
  function processBatch(batch, offset) {
    batch.forEach(function (docId, j) {
      ...
      db.get(docId, docOpts, function (err, res$$1) {
        var result;
        /* istanbul ignore if */
        if (err) {
          result = [{error: err}];
        } else {
          result = formatResult(res$$1);
        }
...
function tryAndPut(db, doc, diffFun) {
  return db.put(doc).then(function (res$$1) {
    return {
      updated: true,
      rev: res$$1.rev
    };
...
var fetch$1 = fetchCookie(nodeFetch);
...
class Changes$1 extends EE {
  constructor(db, opts, callback) {
    ...
    var complete = opts.complete = once$$1((err, resp) => {
      if (err) {
        if (listenerCount$$1(this, 'error') > 0) {
          this.emit('error', err);
        }
      } else {
...
function computeHeight(revs) {
  ...
  traverseRevTree(revs, function (isLeaf, pos, id, prnt) {
    ...
    if (prnt !== undefined) {
      edges.push({from: prnt, to: rev$$1$$1});
    }
    return rev$$1$$1;
  });
...
class AbstractPouchDB extends EE {
  _setup() {
    this.post = adapterFun$$1('post', function (doc, opts, callback) {
    ...
    this.put = adapterFun$$1('put', function (doc, opts, cb) {
      if (typeof opts === 'function') {
        cb = opts;
        opts = {};
      }
    ...
    this.putAttachment = adapterFun$$1('putAttachment', function (docId, attachmentId, rev$$1$$1, blob, type) {
      var api = this;
    ...
    this.removeAttachment = adapterFun$$1('removeAttachment', function (docId, attachmentId, rev$$1$$1, callback) {
    ...
    this.remove = adapterFun$$1('remove', function (docOrId, optsOrRev, opts, callback) {
      var doc;
    ...
    this.revsDiff = adapterFun$$1('revsDiff', function (req, opts, callback) {
      if (typeof opts === 'function') {
        callback = opts;
        opts = {};
      }
      ...
      function processDoc(id, rev_tree) {
        ...
        traverseRevTree(rev_tree, function (isLeaf, pos, revHash, ctx,
          opts) {
            var rev$$1$$1 = pos + '-' + revHash;
            var idx = missingForId.indexOf(rev$$1$$1);
            if (idx === -1) {
              return;
            }
            ...
            if (opts.status !== 'available') {
              addToMissing(id, rev$$1$$1);
            }
          });

        // Traversing the tree is synchronous, so now `missingForId` contains
        // revisions that were not found in the tree
        missingForId.forEach(function (rev$$1$$1) {
          addToMissing(id, rev$$1$$1);
        });
      }
    ...
    this.bulkGet = adapterFun$$1('bulkGet', function (opts, callback) {
    ...
    this.compactDocument = adapterFun$$1('compactDocument', function (docId, maxHeight, callback) {
      this._getRevisionTree(docId, (err, revTree) => {
        ...
        var candidates = [];
        var revs = [];
        Object.keys(height).forEach(function (rev$$1$$1) {
          if (height[rev$$1$$1] > maxHeight) {
            candidates.push(rev$$1$$1);
          }
        });

        traverseRevTree(revTree, function (isLeaf, pos, revHash, ctx, opts) {
          var rev$$1$$1 = pos + '-' + revHash;
          if (opts.status === 'available' && candidates.indexOf(rev$$1$$1) !== -1) {
            revs.push(rev$$1$$1);
          }
        });
    ...
    // compaction
    this.compact = adapterFun$$1('compact', function (opts, callback) {
      if (typeof opts === 'function') {
        callback = opts;
        opts = {};
      }
    ...
    this.get = adapterFun$$1('get', function (id, opts, cb) {
      if (typeof opts === 'function') {
        cb = opts;
        opts = {};
      }
        ...
        if (opts.revs || opts.revs_info) {
          ...
          if (opts.revs) {
            doc._revisions = {
              ...
              ids: path$$1.ids.map(function (rev$$1$$1) {
                return rev$$1$$1.id;
              })
            };
          }
          if (opts.revs_info) {
            ...
            doc._revs_info = path$$1.ids.map(function (rev$$1$$1) {
              pos--;
              return {
                rev: pos + '-' + rev$$1$$1.id,
                status: rev$$1$$1.opts.status
              };
            });
          }
        }
    ...
    this.getAttachment = adapterFun$$1('getAttachment', function (docId, attachmentId, opts, callback) {
    ...
    this.allDocs = adapterFun$$1('allDocs', function (opts, callback) {
      if (typeof opts === 'function') {
        callback = opts;
        opts = {};
      }
    ...
    this.close = adapterFun$$1('close', function (callback) {
      this._closed = true;
      this.emit('closed');
      return this._close(callback);
    }).bind(this);

    this.info = adapterFun$$1('info', function (callback) {
      this._info((err, info) => {
        if (err) {
          return callback(err);
        }
    ...
    this.id = adapterFun$$1('id', function (callback) {
      return this._id(callback);
    }).bind(this);

    this.bulkDocs = adapterFun$$1('bulkDocs', function (req, opts, callback) {
      if (typeof opts === 'function') {
        callback = opts;
        opts = {};
      }
    ...
    this.registerDependentDatabase = adapterFun$$1('registerDependentDatabase', function (dependentDb, callback) {
    ...
    this.destroy = adapterFun$$1('destroy', function (opts, callback) {
...
PouchDB.defaults = function (defaultOpts) {
  let PouchWithDefaults = createClass(PouchDB, function (name, opts) {
    ...
    opts = assign$2$$1({}, PouchWithDefaults.__defaults, opts);
  ...
  PouchWithDefaults.__defaults = assign$2$$1({}, this.__defaults, defaultOpts);
...
function parseRevisionInfo(rev$$1$$1) {
  if (!/^\d+-/.test(rev$$1$$1)) {
    return createError(INVALID_REV);
  }
  var idx = rev$$1$$1.indexOf('-');
  var left = rev$$1$$1.substring(0, idx);
  var right = rev$$1$$1.substring(idx + 1);
  return {
...
function LevelPouch(opts, callback) {
  ...
  var dbStore;
  var leveldownName = res$2$$1(leveldown);
  ...
  api._info = function (callback) {
    var res = {
      ...
      backend_adapter: res$2$$1(leveldown)
    };
    return nextTick$$1(function () {
      callback(null, res);
    });
  ...
  api._close = function (callback) {
    ...
    db.close(function (err) {
      ...
      if (err) {
        ...
        var adapterName = res$2$$1(leveldown);
...
function HttpPouch(opts, callback) {
  ...
  var ourFetch = function (url, options) {
    ...
    options = options || {};
    options.headers = options.headers || new Headers$1();
    ...
    var fetchFun = opts.fetch || fetch$1;
    return fetchFun(url, options);
  };

  function adapterFun$$1$$1(name, fun) {
  ...
  api.id = adapterFun$$1$$1('id', function (callback) {
  ...
  api.compact = adapterFun$$1$$1('compact', function (opts, callback) {
    if (typeof opts === 'function') {
      callback = opts;
      opts = {};
    }
  ...
  api.get = adapterFun$$1$$1('get', function (id, opts, callback) {
  ...
  api.remove = adapterFun$$1$$1('remove', function (docOrId, optsOrRev, opts, cb) {
    var doc;
    if (typeof optsOrRev === 'string') {
      // id, rev, opts, callback style
      doc = {
  ...
  // Get the attachment
  api.getAttachment = adapterFun$$1$$1('getAttachment', function (docId, attachmentId,
                                                            opts, callback) {
    if (typeof opts === 'function') {
      callback = opts;
      opts = {};
  ...
  api.removeAttachment =  adapterFun$$1$$1('removeAttachment', function (docId,
                                                                   attachmentId,
                                                                   rev,
                                                                   callback) {
    var url = genDBUrl(host, encodeDocId(docId) + '/' +
  ...
  api.putAttachment = adapterFun$$1$$1('putAttachment', function (docId, attachmentId,
                                                            rev, blob,
                                                            type, callback) {
    if (typeof type === 'function') {
      callback = type;
    ...
    // Add the attachment
    fetchJSON(url, {
      headers: new Headers$1({'Content-Type': type}),
      method: 'PUT',
      body: blob
  ...
  api.allDocs = adapterFun$$1$$1('allDocs', function (opts, callback) {
    if (typeof opts === 'function') {
      callback = opts;
      opts = {};
    }
  ...
  api.revsDiff = adapterFun$$1$$1('revsDiff', function (req, opts, callback) {
  ...
  function httpQuery(db, fun, opts) {
    ...
    if (typeof fun === 'string') {
      ...
      return db.fetch('_design/' + parts[0] + '/_view/' + parts[1] + params, {
        headers: new Headers$1({'Content-Type': 'application/json'}),
        method: method,
        body: JSON.stringify(body)
      }).then(function (response) {
        ok = response.ok;
...
class Sync extends EE {
  constructor(src, target, opts, callback) {
    ...
    const optsPush = opts.push ? assign$2$$1({}, opts, opts.push) : opts;
    const optsPull = opts.pull ? assign$2$$1({}, opts, opts.pull) : opts;
...
function objectCollate$1(a, b) {
  ...
  for (var i = 0; i < len; i++) {
    // First sort the keys
    var sort = collate$2(ak[i], bk[i]);
    if (sort !== 0) {
      return sort;
    }
    // if the keys are equal sort the values
    sort = collate$2(a[ak[i]], b[bk[i]]);
    if (sort !== 0) {
      return sort;
    }
...
function createAbstractMapReduce$1(localDocName, mapper, reducer, ddocValidator) {
  ...
  function updateViewInQueue(view, opts) {
    ...
    function createIndexableKeysToKeyValues(mapResults) {
      ...
      for (var i = 0, len = mapResults.length; i < len; i++) {
        var emittedKeyValue = mapResults[i];
        var complexKey = [emittedKeyValue.key, emittedKeyValue.id];
        if (i > 0 && collate$2(emittedKeyValue.key, lastKey) === 0) {
          complexKey.push(i); // dup key+id, so make it unique
        }
  ...
  function queryPromised(db, fun, opts) {
    ...
    var updateViewOpts = {
      changes_batch_size: db.__opts.view_update_changes_batch_size || CHANGES_BATCH_SIZE$2
    };
    ...
    if (typeof fun !== 'string') {
      ...
      tempViewQueue$1.add(function () {
        var createViewPromise = createView$1(
          ...
          /* localDocName */ localDocName);
        return createViewPromise.then(function (view) {
          return fin$1(updateView(view, updateViewOpts).then(function () {
            return queryView(view, opts);
          }), function () {
            return view.db.destroy();
          });
...
function setFieldInDoc$1(doc, parsedField, value) {
...
function getKey$1(obj$$1) {
  return Object.keys(obj$$1)[0];
}

function getValue$1(obj$$1) {
  return obj$$1[getKey$1(obj$$1)];
}
      ...
      if (isCombinationalField$1(field)) {
        ...
        Object.keys(matcher).forEach(function (operator) {
          ...
          if (operator === '$gt' || operator === '$gte') {
            ...
            return mergeRegex$1(value, fieldMatchers);
          }
          fieldMatchers[operator] = value;
        });
      }
...
// combine $regex values into one array
function mergeRegex$1(value, fieldMatchers) {
  if ('$regex' in fieldMatchers) {
    // a value could match multiple regexes
    fieldMatchers.$regex.push(value);
  } else { // doesn't exist yet
...
//#7458: execute function mergeAndedSelectors on nested $and
function mergeAndedSelectorsNested$1(obj$$1) {
    for (var prop in obj$$1) {
        if (Array.isArray(obj$$1)) {
            for (var i in obj$$1) {
                if (obj$$1[i]['$and']) {
                    obj$$1[i] = mergeAndedSelectors$1(obj$$1[i]['$and']);
                }
            }
        }
        var value = obj$$1[prop];
        if (typeof value === 'object') {
            mergeAndedSelectorsNested$1(value); // <- recursive call
        }
    }
    return obj$$1;
}
...
//#7458: determine id $and is present in selector (at any level)
function isAndInSelector$1(obj$$1, isAnd) {
    for (var prop in obj$$1) {
        if (prop === '$and') {
            isAnd = true;
        }
        var value = obj$$1[prop];
        if (typeof value === 'object') {
            isAnd = isAndInSelector$1(value, isAnd); // <- recursive call
        }
    }
    return isAnd;
}
...
function createFieldSorter$1(sort) {
  ...
  return function (aRow, bRow) {
    var aFieldValues = getFieldValuesAsArray(aRow.doc);
    var bFieldValues = getFieldValuesAsArray(bRow.doc);
    var collation = collate$2(aFieldValues, bFieldValues);
    if (collation !== 0) {
      return collation;
    }
    // this is what mango seems to do
...
function arrayContainsValue$1(docFieldValue, userValue) {
  return userValue.some(function (val) {
    if (docFieldValue instanceof Array) {
      return docFieldValue.some(function (docFieldValueItem) {
        return collate$2(val, docFieldValueItem) === 0;
      });
    }

    return collate$2(val, docFieldValue) === 0;
  });
}
...
var matchers$1 = {
  ...
  '$eq': function (doc, userValue, parsedField, docFieldValue) {
    return fieldIsNotUndefined$1(docFieldValue) && collate$2(docFieldValue, userValue) === 0;
  },
  ...
  '$ne': function (doc, userValue, parsedField, docFieldValue) {
    return userValue.every(function (neValue) {
      return collate$2(docFieldValue, neValue) !== 0;
    });
  },
  '$in': function (doc, userValue, parsedField, docFieldValue) {
  ...
  '$size': function (doc, userValue, parsedField, docFieldValue) {
    return fieldExists$1(docFieldValue) &&
      Array.isArray(docFieldValue) &&
      arraySize$1(docFieldValue, userValue);
  },
...
function dbFetch(db, path$$1, opts, callback) {
  var status, ok;
  opts.headers = new h({'Content-type': 'application/json'});
  db.fetch(path$$1, opts).then(function (response) {
    status = response.status;
    ok = response.ok;
    return response.json();
  }).then(function (json) {
...
function pick$2(obj$$1, arr) {
  var res = {};
  for (var i = 0, len = arr.length; i < len; i++) {
    var parsedField = parseField$1(arr[i]);
    var value = getFieldFromDoc$1(obj$$1, parsedField);
    if (typeof value !== 'undefined') {
      setFieldInDoc$1(res, parsedField, value);
    }
  }
  return res;
}
...
function uniq$2(arr) {
  ...
  return Object.keys(obj$$1).map(function (key) {
    return key.substring(1);
  });
}
...
function createDeepMultiMapper(fields, emit, selector) {
  return function (doc) {
    if (selector && !matchesSelector$1(doc, selector)) { return; }
    var toEmit = [];
...
function filterInclusiveStart(rows, targetValue, index) {
  ...
  for (var i = 0, len = rows.length; i < len; i++) {
    ...
    if (Math.abs(collate$2(docKey, targetValue)) > 0) {
...
function sortFieldsByIndex(userFields, index) {
  ...
  return userFields.slice().sort(function (a$$1, b) {
    var aIdx = indexFields.indexOf(a$$1);
...
function find$1(db, requestDef, explain) {
  ...
  return getIndexes$1(db).then(function (getIndexesRes) {
    ...
    if ('startkey' in opts && 'endkey' in opts &&
        collate$2(opts.startkey, opts.endkey) > 0) {
